...
from database.dao import admins_dao, stats_dao
...
async def my_stats(callback: CallbackQuery):
    ...
    stats = await stats_dao.get_admin_stats(callback.from_user.id)
    rank, total_ranked = await stats_dao.get_admin_rank(callback.from_user.id)
...
async def admin_ranking(callback: CallbackQuery):
    ...
    rows = await stats_dao.get_admin_ranking()
    try:
