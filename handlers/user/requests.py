...
async def send_request(callback: CallbackQuery, state: FSMContext):
    ...
    text = get_text(language, 'new_request')
...
async def choose_category(callback: CallbackQuery, state: FSMContext):
    ...
    text = get_text(language, 'describe_problem')
    await chat_utils.show(
        callback.bot,
        callback.message.chat.id,
        text,
...
async def get_request(message: Message, state: FSMContext):
    ...
    if len(text) > MAX_REQUEST_LENGTH:
        answer = get_text(language, 'too_long')
        answer = answer.format(limit=MAX_REQUEST_LENGTH)
...
async def my_requests(callback: CallbackQuery):
    ...
    request = requests[0]
    text = requests_utils.format_text(request, language)
    keyboard = user.request_navigation(
        0,
        len(requests),
        request[0],
        request[5],
...
async def cancel_own_request(callback: CallbackQuery):
    ...
    success = await users_dao.cancel_own_request(
        request_id,
        callback.from_user.id
    )
    ...
    if not success:
        if language == 'ru':
            ...
            text = 'Arizani bekor qilib bo‘lmadi.'

        await callback.answer(text, show_alert=True)
        return

    text = get_text(language, 'cancelled')
...
async def user_request_text(message: Message, state: FSMContext):
    ...
    if request is None or request[1] != message.from_user.id or request[6] is None or request[5] != 'В работе':
        text = get_text(language, 'closed')
        text = text.format(id=request_id or '')
        await message.answer(text)
        await state.clear()
        return

    text = get_text(language, 'answer_prefix')
    text = text.format(id=request[0])
    ...
    await message.answer(
        get_text(language, 'request_sent'),
        reply_markup=user.main_keyboard(language)
    )
    await state.clear()
...
async def rate_request(callback: CallbackQuery):
    ...
    success = await admins_dao.add_rating(
        request_id,
        callback.from_user.id,
        rating
    )
    ...
    text = get_text(language, 'rating_saved')
    text = text.format(rating=rating)
    await callback.message.edit_text(
        text,
        reply_markup=user.main_keyboard(language)
    )
