...
async def get_all_requests_page(page, limit):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute('SELECT * FROM requests ORDER BY id ASC LIMIT ? OFFSET ?', (limit, page * limit))
        return await cursor.fetchall()
...
async def get_admin_requests_count(admin_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("SELECT COUNT(*) FROM requests WHERE admin_id = ? AND status = 'В работе'", (admin_id,))
        return (await cursor.fetchone())[0]

async def get_rejected_requests_page(page, limit):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("SELECT * FROM requests WHERE status = 'Отклонено' ORDER BY id ASC LIMIT ? OFFSET ?", (limit, page * limit))
        return await cursor.fetchall()
...
async def take_request(request_id, admin_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("UPDATE requests SET admin_id = ?, status = 'В работе' WHERE id = ? AND status = 'Новая' AND (SELECT COUNT(*) FROM requests WHERE admin_id = ? AND status = 'В работе') < 3", (admin_id, request_id, admin_id))
        await conn.commit()
...
async def complete_request(request_id, admin_id=None):
    ...
    async with aiosqlite.connect('database.db') as conn:
        if admin_id is not None:
            cursor = await conn.execute("UPDATE requests SET status = 'Завершена', completed_at = ? WHERE id = ? AND status = 'В работе' AND admin_id = ?", (completed_at, request_id, admin_id))
        else:
            cursor = await conn.execute("UPDATE requests SET status = 'Завершена', completed_at = ? WHERE id = ? AND status = 'В работе'", (completed_at, request_id))
        await conn.commit()
        return cursor.rowcount > 0
...
async def get_admin_request_by_id(request_id, admin_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("SELECT * FROM requests WHERE id = ? AND status = 'В работе' AND admin_id = ?", (request_id, admin_id))
        return await cursor.fetchone()
...
async def return_rejected_request(request_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("UPDATE requests SET status = 'Новая', admin_id = NULL, reason = NULL WHERE id = ? AND status = 'Отклонено'", (request_id,))
        await conn.commit()
        return cursor.rowcount > 0
...
async def cancel_request(request_id, admin_id, reason):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute("UPDATE requests SET status = 'Отклонено', reason = ? WHERE id = ? AND status = 'В работе' AND admin_id = ?", (reason, request_id, admin_id))
        await conn.commit()
        return cursor.rowcount > 0
...
async def add_admin(admin_id, admin_role, admin_name, priority):
    async with aiosqlite.connect('database.db') as conn:
        try:
            await conn.execute('INSERT INTO admins (admin_id, admin_role, admin_name, priority) VALUES (?, ?, ?, ?)', (admin_id, admin_role, admin_name, priority))
            await conn.commit()
            return True
...
async def get_admin_stats(admin_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute(
            ...
            "FROM requests WHERE admin_id = ?", (admin_id,)
        )
        processed_total, processed_today = await cursor.fetchone()
        cursor = await conn.execute('SELECT COUNT(*), COALESCE(AVG(rating), 0) FROM ratings WHERE admin_id = ?', (admin_id,))
        rating_count, average_rating = await cursor.fetchone()
        cursor = await conn.execute("SELECT COUNT(*) FROM requests WHERE admin_id = ? AND status = 'В работе'", (admin_id,))
        active = (await cursor.fetchone())[0]
    return {'processed_total': processed_total or 0, 'processed_today': processed_today or 0, 'rating_count': rating_count or 0, 'average_rating': average_rating or 0, 'active': active or 0}
...
async def get_admin_ranking():
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute(
            "SELECT a.admin_id, a.admin_name, COALESCE((SELECT AVG(r.rating) FROM ratings r WHERE r.admin_id = a.admin_id), 0), "
            "(SELECT COUNT(*) FROM ratings r WHERE r.admin_id = a.admin_id), "
            "(SELECT COUNT(*) FROM requests q WHERE q.admin_id = a.admin_id AND q.status = 'Завершена') "
            "FROM admins a ORDER BY 3 DESC, 4 DESC, 5 DESC, a.admin_id ASC"
        )
...
async def add_rating(request_id, user_id, rating):
    if rating not in range(1, 6):
        return False
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute('SELECT user_id, admin_id, status FROM requests WHERE id = ?', (request_id,))
        request = await cursor.fetchone()
        if request is None or request[0] != user_id or request[1] is None or request[2] != 'Завершена':
            return False
        try:
            await conn.execute('INSERT INTO ratings (request_id, user_id, admin_id, rating, created_at) VALUES (?, ?, ?, ?, ?)', (request_id, user_id, request[1], rating, datetime.now(timezone.utc).isoformat()))
            await conn.commit()
            return True
        except aiosqlite.IntegrityError:
            return False
...
async def has_rating(request_id, user_id):
    async with aiosqlite.connect('database.db') as conn:
        cursor = await conn.execute('SELECT 1 FROM ratings WHERE request_id = ? AND user_id = ?', (request_id, user_id))
        return await cursor.fetchone() is not None
